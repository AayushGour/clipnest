...
struct ShortcutHintsMatrixTests {
  ...
  private static let capabilityCases: [(name: String, capabilities: HighlightedItemCapabilities)] =
    [
  ...
  func historyTextExact() {
    #expect(
      ShortcutHints.text(
        for: .history, capabilities: HighlightedItemCapabilities(item: makeClipItem(kind: .text)))
