...
5. **Docs updated IN THE SAME CHANGE — always, no exceptions.** If you built, changed, or removed a feature, update every document that describes it before you report done: `docs/`, `README.md`, packaging docs (`debian/README.source`), and — critically — **in-code doc comments that state a current limitation**. A comment saying "X is not yet possible because Y" is a claim with an expiry date: when you fix Y, delete the claim. Never leave it for a follow-up task.
   **Why this is a gate and not a nicety:** on 2026-09-06 stale docs produced three confident, wrong answers in one session. `debian/README.source` still said ONNX Runtime was hard-linked and forced a `Depends:` long after the code moved to `dlopen`, so the main thread told the user OCR "cannot be done" when it was mostly solved. `Package.swift` said a build flag was off "until the .deb vendors `onnxruntime_c_api.h`" when those headers were already vendored. The board listed three Linux P0/P1 bugs as `todo` that were demonstrably fixed. Each was derived from a document rather than the code.
   **Corollary for everyone, every task:** before citing a doc as evidence for a decision, verify it against the code. The code is the source of truth; a doc is a cache that goes stale silently.
