...
**Homebrew cask** (`Casks/clipnest.rb`): a standard `cask` block pointing at the GitHub Releases `.dmg` asset; `version`/`sha256` are rewritten automatically by the release workflow's last step, never hand-maintained (`Casks/clipnest.rb:1-9`, `.github/workflows/release.yml:145-166`). `zap trash:` lists the exact three/four on-disk locations Clipnest ever writes to, for a clean `--zap` uninstall (`Casks/clipnest.rb:23-28`).
