...
// P2-C (Linux port): wrapped in `#if os(macOS)` — `Vision.VNRecognizeTextRequest`
// is Apple-only. Moved verbatim into `Platform/macOS/`, staying in the
// `ClipnestCore` module (see `SwiftDataClipStore.swift`'s top doc comment for
// the module-split rationale, which applies to every file this task moves).
// `import os` replaced with the portable `ClipnestLogger` shim
// (`Logging.swift`) per P2-C's required logging conversion — `os.Logger`'s
// `privacy:` interpolation is Apple-only, so it can't be the shim's surface;
// this file's one log call site was already metadata-only
// (`String(describing: error)`), preserved unchanged below.
#if os(macOS)
  import CoreGraphics
  import Foundation
  import ImageIO
  ...
  public struct VisionTextRecognizer: TextRecognizing {
    ...
    private static func performVisionRequest(
      ...
      do {
        ...
        logger.error(
          "VisionTextRecognizer: request failed (\(String(describing: error)))"
        )
        continuation.resume(returning: nil)
      }
    }
