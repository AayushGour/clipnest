...
// P2-C (Linux port): wrapped in `#if os(macOS)` — `CoreGraphics`/`ImageIO`
// decode + re-render is Apple-only (`CryptoKit` alone is not: `BlobStore`
// already ports it via `#if canImport(CryptoKit) / #else import Crypto`, see
// that file's P1-T8 note — but this hasher's canonicalization algorithm is
// inherently `CGImage`/`CGContext`-based, so the whole type stays macOS-only
// rather than chasing a Linux equivalent here). Moved verbatim into
// `Platform/macOS/`, staying in the `ClipnestCore` module (see
// `SwiftDataClipStore.swift`'s top doc comment for the module-split
// rationale). No `os.Logger` call sites existed in this file, so no logging
// conversion was needed here.
#if os(macOS)
  import CoreGraphics
  import CryptoKit
  import Foundation
  ...
  public struct CoreGraphicsImagePixelHasher: ImagePixelHashing {
    ...
    private static func appendPreimageHeader(
      width: UInt32, height: UInt32, into hasher: inout SHA256
    ) {
      ...
      while y < height {
        ...
        hasher.update(
          bufferPointer: UnsafeRawBufferPointer(
            start: UnsafeRawPointer(bandBytes), count: byteCount)
        )
  ...
  /// P2-C (Linux port): the macOS-side default for `ImagePixelHashing`
  /// collaborators, resolved through `PlatformDefaults` rather than a bare
  /// `= CoreGraphicsImagePixelHasher()` default-argument literal — see
  /// `Platform/PlatformDefaults.swift`'s doc comment for why (a Linux build
  /// can't name an Apple-only type in a default argument, even indirectly, so
  /// every such default is routed through a `PlatformDefaults` member whose
  /// macOS value lives beside its implementation, here).
  ///
  /// Not yet wired to any call site by this task: `SwiftDataClipStore.init`'s
  /// own `imagePixelHasher: any ImagePixelHashing = CoreGraphicsImagePixelHasher()`
  /// default lives inside this same `#if os(macOS)` file (this task's move),
  /// so it never reaches a Linux build regardless and was left as a verbatim,
  /// no-logic-change move per this task's scope. `PasteboardReader.swift`
  /// (`Clipboard/`, a parallel task's file, out of this task's edit scope) has
  /// an identical bare default — but that file still unconditionally
  /// `import AppKit`s today, so it isn't Linux-portable yet either; once it is,
  /// its default should switch to `PlatformDefaults.imagePixelHasher` instead
  /// of reaching back into this file, and `PlatformDefaults` will need a
  /// non-Apple `#else` fallback added alongside this extension at that time
  /// (see the doc comment on `PlatformDefaults.swift` for the "portable no-op,
  /// never used in production" pattern the Linux side follows). Added
  /// proactively so that future wiring is a one-line call-site change, not a
  /// second trip through this file.
  extension PlatformDefaults {
    public static var imagePixelHasher: any ImagePixelHashing {
      CoreGraphicsImagePixelHasher()
    }
  }
#endif
