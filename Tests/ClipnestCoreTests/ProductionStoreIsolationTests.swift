...
// SwiftData-backed persistence (`@Model`, `ModelContainer`,
// `SwiftDataClipStore`/`SwiftDataSnippetStore`), which is itself
// `#if os(macOS)` in ClipnestCore — so the whole file is gated the same way,
// nesting the existing `#if DEBUG` gate below inside it.
#if os(macOS)
  import Foundation
  import SwiftData
  import Testing
